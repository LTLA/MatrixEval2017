...
Each \code{HDF5Matrix} instance provides methods to mimic a \revised{\code{matrix}} object, allowing users to manipulate the matrix in real time without needing to load all of the data into memory.
...
This means that very large data sets can be accessed in C++ with the same code that is used for \revised{dense} and sparse matrix representations.
However, data access is inevitably slower than that from a \revised{dense} matrix as the data need to be read from \revised{file} at regular intervals.
...
In the contiguous layout, raw data are flattened into a one-dimensional array analogous to column-major storage of \revised{dense} matrices in memory.
...
We considered the multiplication of two square matrices of order ranging from 500-2000, using dense matrices containing random values from a normal distribution;
...
For comparison, timings were also obtained for the \code{\%*\%} operator in R (from the base package, for dense matrices; 
from the \textit{Matrix} package, for sparse matrices;
or from the \textit{DelayedArray} package, for HDF5-backed matrices).
We observed that the performance of our implementation for dense and HDF5-backed matrices was comparable to the representation-specific \code{\%*\%} operators,
...
The contiguous layout is the fastest as \revised{all relevant data can be obtained in one read from file} per column \revised{(Figure~3a)}.
...
The timing results are reversed for row access, where row-wise chunking is the layout that provides the best performance with \beachmat{} \revised{(Figure~3b)}.
This is expected as only one \revised{chunk} read is required per row, compared to multiple reads with column-wise chunking \revised{or a contiguous layout}.
Rectangular chunking is only slightly slower than row-wise chunking when the chunk cache is optimized.
