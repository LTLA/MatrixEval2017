...
This is also slightly faster than column access through} the \textit{RcppArmadillo} \revised{and \textit{RcppEigen} packages \cite{eddelbuettel2014arma,bates2013fast}, which} also handle sparse matrices via the \revised{\code{sp\_mat} and \code{MappedSparseMatrix} classes, respectively}.
\revised{However, it should be noted that part of \beachmat{}'s speed-up with \code{get\_const\_col\_indexed()} is driven by the fact that the zero entries of the sparse matrix are completely ignored.
