...
The statistical programming language R \cite{R}, \revised{in conjunction with the Bioconductor project \cite{gentleman2004bioconductor}}, provides \revised{the pre-eminent} platform for \revised{exploring and analyzing genomics} data.
...
This \revised{copy-on-access} strategy provides the greatest flexibility for downstream applications, by allowing in-place modifications to vector elements and guaranteeing contiguous data storage. 
\revised{When accessing column data from dense and sparse matrices, it is also possible to avoid copying altogether for greater efficiency (Supplementary Section~XX).}
\revised{Finally,} a request for a specific entry of the matrix will directly return the corresponding data value.
...
\revised{\beachmat{} also supports} sparse matrices in the compressed sparse column-orientated (CSC) format (Supplementary Section~\suppsecsparse{}, Supplementary Figure~\suppfigsparseschem{}), 
implemented in the \texttt{dgCMatrix} class from \textit{Matrix}.
This representation only stores non-zero values in \revised{RAM}, which \revised{is highly efficient for assays that yield many zeroes, e.g., scRNA-seq}.
However, the more complex format means that data access from sparse matrices \revised{can be} slower than \revised{the equivalent dense} matrices (Supplementary Figure~\suppfigsparsecol{}).
We use a novel caching strategy to improve the efficiency of accessing data from consecutive rows by avoiding unnecessary binary searches (Figure~\ref{fig:sparse}, Supplementary Section~\suppsecsparse{}).
\revised{This provides a 4-fold increase in speed compared to a naive approach, as well as faster row-level data access than the \textit{RcppArmadillo} and \textit{RcppEigen} APIs \cite{eddelbuettel2014arma,bates2013fast}.}
...
as well as functions to convert to file layouts that are \revised{well-suited} for random row or column access (Supplementary Figures~\suppfighdfrandom{}, Supplementary Section~\suppseclayoutrandom{}).

\begin{figure}[btp]
    \begin{center}
        \includegraphics[width=\textwidth]{Figure3.pdf}
    \end{center}
    \caption{\revised{Timing of \beachmat{}-based access to consecutive columns or rows of a \code{HDF5Matrix} object constructed with different HDF5 file layouts, 
...
\subsection*{Introduction to single-cell RNA sequencing}
