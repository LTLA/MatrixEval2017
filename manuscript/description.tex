...
This includes:
\begin{itemize}
    \item \revised{Dense} R matrices, constructed with the base \texttt{matrix} function or using the \texttt{dgeMatrix} class from the \textit{Matrix} package.
    ...
    \item Sparse matrices in compressed sparse column-orientated format (Supplementary Section~\suppsecsparse{}, Supplementary Figure~\suppfigsparseschem{}), 
        ...
        However, the more complex format means that data access from sparse matrices is generally slower than \revised{dense} matrices (Supplementary Figure~\suppfigsparsecol{}).
...
We note that an alternative to using \beachmat{} is to write C++ code for \revised{dense} matrices and apply it to submatrices (or ``blocks'') of a given input matrix.
