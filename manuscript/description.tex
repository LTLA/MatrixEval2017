...
\begin{flushleft}
{\Large
\textbf\newline{beachmat: a Bioconductor C++ API for accessing genomics data from a variety of R matrix types} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
\revised{The combination of the statistical programming language R \cite{R} and the open-source Bioconductor project \cite{gentleman2004bioconductor} represents the pre-eminent} platform for \revised{exploring and analyzing genomics} data.
...
\revised{We note that \beachmat{} does not provide any direct support for high-level matrix operations such as multiplication or factorization.
...
This suggests that the lack of support for matrix operations in \beachmat{} will not be a major hindrance to its uptake in the Bioconductor development framework.
In fact, a representation-agnostic API may not be practical for matrix operations that are not scalable for very large matrices.
The decision to use faster approximate approaches must be made by the developer, not by a low-level API like \beachmat{}.}
