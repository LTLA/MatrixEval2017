...
We use a simple strategy to convert an existing chunked HDF5 file to a layout using row-based chunks.
...
We consider a stretch of $\myceil{Q_2/Q_1}$ chunks spanning the same $P_1$ rows. 
These chunks are loaded from the original file into memory and written into a new file as $P_1$ row-wise chunks.
This is repeated with the next stretch of chunks until all data are copied from the original to the new file.
Similarly, to create column-wise chunks of size $P_2$, we consider the stretch of $\myceil{P_1/P_2}$ chunks spanning the same $Q_1$ columns.
These are loaded into memory and written into a new file as $Q_1$ column-wise chunks.
The idea is to load read and write entire chunks as much as possible, reducing the number of redundant disk accesses.
Disk reads/writes are minimized when $P_2$ and $Q_2$ are multiples of $P_1$ and $P_2$, respectively.
