...
We set $S=8000000$ to simplify the later divisions, which corresponds to an in-memory size of 64 MB.
...
We emphasize that the chunk dimensions are not chosen automatically by \textit{beachmat} but must be manually specified.
Obviously, the choice of chunk dimensions is irrelevant when accessing data from existing HDF5 files where the layout is fixed.
For data output from \textit{beachmat}, it is possible to control the chunk dimensions of the new HDF5 file.
However, the dimensions still need to be chosen by the user/developer for optimal performance.
This is because \textit{beachmat} cannot know the anticipated access pattern at the time of file creation.
