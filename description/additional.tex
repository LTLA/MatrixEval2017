...
The simple matrix is easy to manipulate and the time required to access \revised{all of its data elements} is linear with respect to the number of rows/columns (Supplementary Figure~\ref{fig:basetime}).
...
This constructs an in-memory \code{big.matrix} object that contains external pointers pointing to a \revised{file in which the data are stored.}
