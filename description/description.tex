...
The simple matrix is easy to manipulate and the time required for data access is linear with respect to the number of rows/columns (Supplementary Figure XXX).
...
Both row and column access via \beachmat{} require 20-50\% more time compared to the reference (Figure XXX).
...
In contrast, the reference simply iterates across the original data, avoiding the overhead of creating a new copy.
...
In the CSC format, non-zero entries are sorted in order of increasing $x + N_ry$.
This means that all entries from the same column are now grouped together.
We refer to each column-based group as $G_y$, the entries of which are sorted internally in order of increasing $x$.
The reordered sequence of entries is further compressed by discarding $y$ from each triplet.
All entries from the same column are at consecutive locations of the sequence, so only the start position of $G_y$ on the sequence needs to be stored for each column.
...
The CSC format simplifies data access by enforcing structure on the non-zero entries.
When accessing a particular column, all corresponding entries in $G_y$ can be quickly extracted by taking the relevant part of the ordered sequence.
For low-density sparse matrices, column access via \beachmat{} is even faster than access from simple matrices (Figure XX).
This is because only a few non-zero entries need to be copied, while the rest of the \code{Vector} can be rapidly filled with zeroes.
...
This avoids the need to perform a new binary search and reduces the row access time substantially (Figure XX).
...
Despite these optimisations, consecutive row access with sparse matrices in \beachmat{} remains slower than that with simple matrices.
...
These results justify the use of \beachmat{} for data access.
