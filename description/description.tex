...
Our use of copying is deliberate as it ensures that the API is consistent across matrix representations -- for example, file-based representations must copy the data to a new location in memory.
...
We note that, for highly sparse data, it is possible to design efficient algorithms that avoid iterating over the zeroes.
...
In contrast, the default acccess methods will return all values of a row/column to keep the API consistent with other non-sparse representations.
...
Writing, testing and maintaining two different versions of the same code is a heavy burden on the developer, and precisely what the use of \beachmat{} was intended to avoid.
...
To demonstrate the utility of \beachmat{} for faciliting analyses of large data sets, we converted several functions in the \textit{scater} \cite{scater2017mccarthy} and \textit{scran} packges \cite{lun2016stepbystep} to use the \beachmat{} API in the C++ code.
We then applied these functions to the 1 million neuron data set from 10X Genomics (see Methods).
First, we called cell cycle phase with the \code{cyclone} method \cite{scialdone2015computationalt}, 
...
We identified a number of HVGs.
