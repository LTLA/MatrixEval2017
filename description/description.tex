...
\newcommand{\suppfighdflayout}{3-4}
\newcommand{\suppfigtenx}{5}
...
We note that, by taking advantage of the file-backed \code{HDF5Matrix} class, this analysis can be conducted in reasonable time on a desktop with modest specifications (see Methods).
...
\subsection*{Machine specification details}
All timings and analyses were performed on a Dell OptiPlex 790 desktop with an Intel Core i5 processor and 8 GB of RAM, running Ubuntu 14.04.5 with R 3.4.0.
...
and 5 hours for PCA.
Only the phase assignment step was parallelized, and greater speed could easily be achieved for other steps with multiple cores (at the cost of memory, as each core has its own chunk cache).
