...
We observed a 10-fold increase in the time required to access each column (Figure~Y), and a XX-fold increase in the time required to access each row.
...
This suggests that the \code{HDF5Matrix} representation should be used sparingly -- smaller data sets should use alternative representations wherever possible.
...
In the contiguous layout, raw data are stored next to each other in the file, analogous to column-major storage of simple matrices in memory.
In the chunked layout, data are arranged into ``chunks'' of a pre-defined size.
For example, in a row-chunked layout, all values in the same row of a matrix would constitute one chunk.
Each chunk is always read (or written) in its entirety, even when only a portion of the chunk is requested.
Chunking is required for compression on disk and fast access to data -- \textit{provided that the layout is consistent with the expected access pattern}.

We demonstrate the effect of the choice of file layout on access to HDF5-backed data.
For column access, we consider the performance of \beachmat{} with a contiguous layout, column-wise chunks (i.e., each column is a chunk) with compression, and row-wise chunks with compression.
The contiguous layout is the fastest as only one disk read needs to be performed per column (Figure~YY).
...
For row access, row-wise chunking is the layout that provides the best performance with \beachmat{} (Figure~YY).
...
This involves some computational overhead due to the need for (de)compression at each read/write.
Despite this, \beachmat{} access times drop with sparser matrices because the compression becomes more effective, reducing the amount of data that needs to be read from disk (Figure~YY).
Obviously, the size of the HDF5 file also decreases with compression (Figure~YY), which reduces the risk that some disk usage limit will be exceeded during the course of an analysis.
...
\section*{Discussion}
% Need some words about bigmemory.
